...
import { fetchProjects, createProject, deleteProject, uploadProjectImage } from "../lib/projects";
...
export default function Projects() {
  ...
  async function pickImage() {
    ...
    setPickedFile({
      uri: asset.uri,
      name: asset.fileName ?? `project-${Date.now()}.jpg`,
  ...
  async function handleSubmit() {
    ...
    setSubmitting(true);
    try {
      const url = await uploadProjectImage(pickedFile);
      const created = await createProject({ title: title.trim(), img: url, description: description.trim(), year: year.trim() });
  ...
  return (
    <>
      <Stack.Screen options={{ headerShown: true, title: "معرض الأعمال" }} />
      <SafeAreaView style={styles.container} edges={["bottom"]}>
        {loading ? (
          ...
          <ScrollView contentContainerStyle={styles.content}>
            ...
            <Text style={styles.sectionTitle}>مشاريعك</Text>
