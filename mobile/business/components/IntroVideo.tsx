 ...
 *
 * This is the Business App's counterpart of mobile/client's
 * components/IntroVideo.tsx — same mechanism, its own asset (assets/intro.mp4
 * here is the client's reveal recoloured to this app's blue, #005578, so the
 * two apps read as related but distinct the moment either one opens).
 *
 ...
 * done — so the app's own startup work (fonts, the stored staff session, the
 * forced-update check) runs UNDERNEATH the animation instead of after it. On a
 * warm boot the two finish together and the intro costs nothing; on a slow one
 * it covers a wait staff would have spent staring at a static splash anyway.
 *
 * Its position in the tree is also why the parent renders it from ONE place
 * rather than from an early `return null`: moving between branches would
 * unmount and remount this component, and a remounted <VideoView> restarts
 * its player from frame zero — the animation would visibly stutter back to
 * the beginning the moment the session resolved.
 *
 ...
 * Every step lands on white, and the asset itself is a mark on white, so
...
export default function IntroVideo({ onDone }: { onDone: () => void }) {
  ...
  // `playToEnd` — none of them may leave staff looking at a white screen.
