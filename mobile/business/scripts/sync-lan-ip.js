#!/usr/bin/env node
/**
 * Auto-corrects the LAN IP baked into this app's .env before every dev
 * start — see the "prestart" hook in package.json. Identical copy of
 * mobile/client's own script (see phase 1 of the mobile plan for why this
 * one wasn't extracted into @alassema/mobile-shared: it's a build-time
 * Node script, never bundled into either app, so there's no runtime
 * duplication to avoid — only a one-line comment to keep in sync.
 *
 * ── Why this exists ──────────────────────────────────────────────────────
