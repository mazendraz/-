...
export default function CatalogError({ message }: { message?: string }) {
  ...
  return (
    <div className="bg-surface-container-lowest rounded-2xl shadow-bloom p-12 text-center max-w-lg mx-auto">
      ...
      <h2 className="font-bold text-[18px] text-on-surface mb-1.5">{t(locale, "catalog_error_title")}</h2>
      <p className="text-[14px] text-outline mb-6 leading-relaxed">
        {message ?? t(locale, "catalog_error_body")}
      </p>
      <button
      ...
      >
        <span className="material-symbols-outlined text-[18px]">refresh</span> {t(locale, "catalog_error_retry")}
      </button>
    </div>
  );
}
