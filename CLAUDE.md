...
## 📱 تحديث الموبايل (OTA) — لازم بأمر واحد بس

عشان تنشر تحديث OTA للتطبيق (TestFlight/الإنتاج)، من `mobile/client`:

```bash
npm run update:production -- "وصف التعديل"
```

**ماتشغّلش `eas update` على طول أبدًا.** ده مش تفضيل — في 2026-09-04 اتنشر OTA
على برانش `production` وهو شايل `http://192.168.1.10:3000/api/v1` (لابتوب
المطوّر) كـ API، وكل تليفون على TestFlight بقى بيكلّم الجهاز ده بدل السيرفر.

**السبب مش إن حد نسي يظبط `.env`** — `.env` اتظبط صح، والـ CLI طبع إنه حمّله.
المشكلة إن `EXPO_PUBLIC_*` بتتحقن في البandل وقت الـ **transform**، وMetro
بيكاش الـ transform في `%TEMP%/metro-cache` بمفتاح **مش فيه قيم المتغيرات دي**.
يعني أول قيمة حقنت `index.ts` بتفضل تتقدّم من الكاش مهما غيّرت في `.env` بعد كده.
اتأكد عمليًا في الاتجاهين: `.env` تطوير → بandل إنتاج، و`.env` إنتاج → بandل تطوير.

عشان كده الحل مش "افتكر تظبط `.env`". السكربت
`mobile/client/scripts/publish-production-update.js` بيعمل:

1. بياخد بيانات الإنتاج من `eas.json` → `build.production.env` (نفس القيم اللي
   `eas build --profile production` بيستخدمها، فمستحيل يختلفوا).
2. بيرفض لو الـ API URL مش `https://al-assema.tech/api/v1` بالظبط.
3. بيعمل export والكاش متمسوح + `.env` متعطّل (`EXPO_NO_DOTENV=1`) + أي
   `EXPO_PUBLIC_*` موروث من الشِل متشال.
4. **بيقرا البandل نفسه** ويتأكد إن مفيش أي عنوان LAN وإن الـ API هو المعتمد بس.
5. بينشر بـ `--skip-bundler --input-dir`، يعني اللي اترفع هو بالظبط اللي اتفحص.
6. بيمسح كاش Metro بعد الرفع، عشان جلسة التطوير الجاية ماتورثش قيم الإنتاج.

> ⚠️ الخطر بيشتغل في الاتجاهين: كاش قديم ممكن يخلّي `npm start` المحلي يكلّم
> **قاعدة بيانات الإنتاج**. لو شكيت في حاجة، `npx expo start --clear`.
...
بس الأقفال لسه بتحصل لأسباب تانية خالص: أمر git اتقطع في نصه (timeout / إلغاء)، VS Code،
الأنتي فيروس، أو أداة بتشتغل على الريبو من بره ويندوز. فالإجراء ده لسه ساري — لو ظهر خطأ
`index file corrupt` أو `index.lock`:
...
> **لو `rm` رفض بـ `Operation not permitted`:** يبقى الأمر شغّال من بره ويندوز (جسر
> Cowork مش بيقدر يمسح ملفات على المجلد المتصل). انقل الملف لـ `_to_delete/` بدل ما تمسحه،
> وشغّل git من ويندوز نفسه لما تقدر — الـ **push** تحديدًا لازم من ويندوز، الجسر مالوش نت.
> نفس الجسر بيسيب كمان ملفات `tmp_obj_*` جوه `.git/objects/` — مضرّتها صفر، بس انقلها لـ
> `_to_delete/` عشان الريبو يفضل نضيف.
