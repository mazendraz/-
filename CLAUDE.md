...
## 📍 مكان الريبو + أقفال git

الريبو على **`F:\العاصمة`** — **بره OneDrive**. (كان قبل كده جوه `OneDrive\Desktop`
واتنقل؛ ده كان بيلخبط git: index تالف، أقفال عالقة، ملفات بترجع لمكانها.)

**ماتنقلوش تاني لأي مجلد متزامن سحابيًا** (OneDrive / Dropbox / Google Drive) — ولا تعمل
نسخة تانية شغّالة منه هناك.

الأقفال لسه ممكن تحصل من VS Code أو الأنتي فيروس (آخر حالة 2026-08-15)، فالإجراء ده لسه
ساري: لو ظهر خطأ `index file corrupt` أو `index.lock` —
...
`scripts/ship.sh` بينضّف قفل الـ index لوحده قبل أي رفع. وبعد أي عملية git، اتأكد بـ
`git status` إن النتيجة زي المتوقع قبل ما تكمّل.
